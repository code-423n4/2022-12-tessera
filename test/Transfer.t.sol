...
contract TransferTest is TestUtil, NFTReceiver {
    ...
    function testMeasureGasERC20() public {
        ...
        bytes memory data = abi.encodeCall(
            ...
            (address(erc20), alice, 10)
        );
        (bool success, bytes memory response) = address(transferTarget).delegatecall(data);
        if (!success) {
    ...
    function testMeasureGasERC20_Reference() public {
        ...
        (bool success, bytes memory response) = address(transferReference).delegatecall(data);
        if (!success) {
            if (response.length == 0) revert();
            _revertedWithReason(response);
        }
    ...
    function testMeasureGasERC721() public {
        ...
        bytes memory data = abi.encodeCall(
            ...
            (address(erc721), address(this), alice, 100)
        );
        (bool success, bytes memory response) = address(transferTarget).delegatecall(data);
        if (!success) {
            if (response.length == 0) revert();
    ...
    function testMeasureGasERC1155() public {
        ...
        bytes memory data = abi.encodeCall(
            ...
            (address(erc1155), address(this), alice, 1, 10)
        );
        (bool success, bytes memory response) = address(transferTarget).delegatecall(data);
        if (!success) {
            if (response.length == 0) revert();
    ...
    function testMeasureGasBatchERC1155() public {
        ...
        bytes memory data = abi.encodeCall(
            ...
            (address(erc1155), address(this), alice, ids, amounts)
        );
        (bool success, bytes memory response) = address(transferTarget).delegatecall(data);
        if (!success) {
            if (response.length == 0) revert();
